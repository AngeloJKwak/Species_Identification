...
# load in data 
data<-fread("./project/volume/data/raw/Gene_data.csv")
...
#ggplot(pca_dt,aes(x=PC1,y=PC2,col=species))+geom_point()
...
submit<- cluster_prob[,.(V1,V2,V3)]
submit$id <- tsne_dt$id

setcolorder(submit, c('id','V1','V3','V2'))

setnames(submit, old = "V1", new = "species1")
setnames(submit, old = "V2", new = "species3")
setnames(submit, old = "V3", new = "species2")

#now we can write out a submission
fwrite(submit,"./project/volume/data/processed/submit_gmm.csv")
